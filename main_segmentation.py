...
def load_segmentation_model():
    try:
        if not os.path.exists(MODEL_PATH):
            ...
            url = "https://drive.google.com/file/d/1JwVsxKFaUMyxRgMUTE-yhpKZMqVU7P3i"
...
def main():
    ...
    if model is None:
        st.error("Failed to load segmentation model. Please check the model file.")
        return
        ...
        if st.button("Run Segmentation"):
            with st.spinner("Processing image..."):
                try:
                    ...
                    mask = prediction[0]  # أول قنال من الـ output
